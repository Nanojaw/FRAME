...
impl<'a> splitter<'a> {
    ...
    pub fn next_char(&mut self, skip: bool, allow_eof: bool) -> Result<Option<char>, String> {
        ...
        if c.is_none() {
            return Err(format!("Unexpected EOF character in line: {} at char {}", self.line_count, self.position_in_line));
        }
...
pub fn split_file(splitter: &mut splitter) -> Result<Vec<block::Block>, String> {
    let blocks: Vec<block::Block> = vec![];
    ...
    while c.is_some() {
        if c.unwrap().is_alphabetic() {
            ...
            } else if let instr_type::InstrType::WithBody = instr_type {
